...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob:drop_prob})
        print('Saving model...')
...
def vector_loss():
    ...
    if args.dynamic_lr:
        ...
        lr=args.lr
    dump=open(args.file,'a')
    dump.write('#configure: vector=True,out_dim=%d,batch_size=%d,drop_prob=%.4f,mistake_rate=%.4f\n'%(out_dim,batch_size,drop_prob,mistake_rate))
    #################################################################
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            #print('label:',np_labels[0]);print('input:',input_np_labels[0]);embed()
            if i % 100 == 0:
                y_conv_val=sess.run(y_conv,feed_dict={
                    x: batch[0], keep_prob: 1.0})
                # get acc
                sum_labels=np.zeros([batch_size,num_label])
                train_acc=0
